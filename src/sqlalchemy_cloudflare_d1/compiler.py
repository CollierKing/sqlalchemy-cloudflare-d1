...
from sqlalchemy.sql.sqltypes import String, Text, Integer, Numeric, DateTime, Boolean, TypeDecorator
...
class CloudflareD1DDLCompiler(compiler.DDLCompiler):
    ...
    def visit_create_table(self, create, **kw):
        ...
        for constraint in table.constraints:
            if constraint.name is not None or not isinstance(constraint, schema.PrimaryKeyConstraint):
                text += separator
    ...
    def visit_create_index(self, create, **kw):
        ...
        text += ", ".join(
            preparer.quote_identifier(c.name) for c in index.columns
        )
        text += ")"
